...
if [ -f "./ptero-manager.sh" ]; then
    ...
    sudo curl -fsSL https://raw.githubusercontent.com/phdassice/pterodactyl-manger/main/ptero-manger/ptero-manager.sh -o "$INSTALL_DIR/$MANAGER_NAME" || {
...
# 安裝快速命令工具（可選）
if [ -f "./quick-commands.sh" ]; then
    echo -e "${YELLOW}從當前目錄複製快速命令工具...${NC}"
    sudo cp ./quick-commands.sh "$INSTALL_DIR/$QUICK_NAME"
    sudo chmod +x "$INSTALL_DIR/$QUICK_NAME"
    echo -e "${GREEN}✓ 快速命令工具已安裝${NC}"
elif curl -fsSL https://raw.githubusercontent.com/phdassice/pterodactyl-manger/main/ptero-manger/quick-commands.sh -o /tmp/quick-commands.sh 2>/dev/null; then
    echo -e "${YELLOW}從 GitHub 下載快速命令工具...${NC}"
    sudo mv /tmp/quick-commands.sh "$INSTALL_DIR/$QUICK_NAME"
    sudo chmod +x "$INSTALL_DIR/$QUICK_NAME"
    echo -e "${GREEN}✓ 快速命令工具已安裝${NC}"
else
